...
export default function HeroTransition() {
  ...
  // Stable ref callbacks — created once, never change between renders
  ...
  // ── Count-up animation (identical to Hero.tsx) ─────────────
  useEffect(() => {
    let rafId: number;
  ...
  useEffect(() => {
    ...
    function measureA() {
      ...
      // transform-origin at A's horizontal center, vertical center
      anchor.style.transformOrigin = `${aRect.left - anchorRect.left + aRect.width / 2}px center`;
    }

    function update() {
      // progress relative to the outer container, 0→1 over 200vh
      ...
      // ── Scale (applied to .wm-anchor) ──────────────────────
      let scale: number;
      if (raw <= 0.25)      scale = 1 + p1 * 4;        // 1→5
      else if (raw <= 0.78) scale = 5 + p2 * 7;        // 5→12
      else                   scale = 12 + p3 * 32;      // 12→44

      // ── A rotation — ease-in (t²) in P2, continues in P3 ──
      let aRotate: number;
      if (raw <= 0.25)      aRotate = 0;
      else if (raw <= 0.78) aRotate = p2 * p2 * 540;   // 0→540deg ease-in
      else                   aRotate = 540 + p3 * 900;  // 540→1440deg

      // ── A color — white → #00A7E1 over P2 ─────────────────
      // #00A7E1 = rgb(0,167,225); delta from white: r:-255, g:-88, b:-30
      let aColor: string;
      if (raw <= 0.25) {
      ...
      // ── P3: bg flood (canvas→cyan), content fade, work veil ─
      // bg overlay interpolates from transparent (#000 at opacity 0) → #00A7E1
      ...
      // content fades out in P3
      content!.style.opacity = raw < 0.78 ? "1" : String(Math.max(0, 1 - p3));

      // work overlay (canvas dark) arrives at the tail of P3
  ...
  return (
    <div ref={outerRef} className="hero-outer">
      ...
      >
        ...
        {/* P3 bg flood: canvas → #00A7E1 (below content, above blobs) */}
        <div
          ref={bgOverlayRef}
          aria-hidden="true"
          style={{
        ...
        >
          ...
          {/* Mobile wordmark — CSS-only drop-in, hidden on ≥md */}
          <div
          ...
          >
            {MOBILE_WORDMARK.map(({ letter, color, delay }) => (
              <span
                ...
                style={{ ...WORDMARK_STYLE, color, animationDelay: delay }}
              >
                {letter}
              </span>
            ))}
          ...
          {/* Desktop wordmark — scroll-driven, hidden on mobile */}
          <div
            className="hidden md:flex items-end justify-center select-none"
            style={{ lineHeight: 0.85, minHeight: "clamp(102px, 18.7vw, 238px)" }}
          >
            {/*
              .wm-anchor: scale applied here; transform-origin measured to A's center.
              A rotates independently inside.
            */}
            <div
              ref={anchorRef}
          ...
          {/* Tagline + CTA — fades out in P1 via scroll JS, initial reveal via React state */}
          <div
            ref={subRef}
        ...
        {/* Work veil — canvas dark overlay, arrives at end of P3 for seamless handoff */}
        <div
          ref={workOverlayRef}
          aria-hidden="true"
          style={{
